...
active `config.json`, and reuses its camera and rotation settings.
Use `--data-dir`, `--models`, `--camera-device`, or `--rotation` only for an
intentional controlled comparison.
