...
1. 相机仅在认证开始后激活。配置了 DevicePath 时必须使用指定设备，枚举不到就失败，禁止静默回退。曝光/增益调谐须观察参数生效：方向正确的亮度响应后，经 ≥200ms 稳定窗口与低漂移趋势可提前结束；无响应证据仍至少等 900ms，持续漂移不确认。确认手动曝光后优先补增益，必要时才逐档延长曝光；亮度余量足够时回收慢曝光至 -5（约31ms）的偏好档位，不牺牲人脸亮度底线。保留曝光上行帽及暗光例外，稳定结果写入 `camera_tune.state` 供下次重放；取消、丢脸或未稳定时不覆盖旧记录。无脸且画面持续大面积饱和时允许有限下调曝光恢复检测，仍须等待驱动稳定；无脸恢复不保存参数，检测恢复后交回人脸调节。预环触发帧无人脸（迟到入场/暗脸漏检）或启动后亮度漂移时，认证循环在**首个计帧之前**另有预算 2 次的迟到曝光纠正（触发帧作废重抓；纠正期间活体三计时经 `LivenessTiming::Suspend/Resume` 平移暂停，15s 全局时限不暂停；计帧开始后不再改传感器参数）。AE 转手动后增益才可可靠评估，设备断电可能静默复位。具体参数、预览看护与实验记录按需查阅 [`face-service.md`](docs/modules/face-service.md#认证主循环)。
