...
<p align="center">
  ...
  <a href="https://github.com/Link2323/FaceLogin/releases"><img src="https://img.shields.io/badge/version-1.8.0--multi--angle%20(Link2323%20fork)-green" alt="Version"></a>
</p>
...
解锁耗时主要取决于摄像头启动速度与 CPU 性能。实测参考（按键触发 → 解锁成功）：

| 机器类型 | 端到端解锁 | 实测机型 |
|---|---|---|
| 高性能桌面 CPU | **~0.8s** | Ryzen 9 7945HX（16C/32T） |
| 性能级笔记本 | **~1.0s** | i7-1360P（6P+8E） |

> 无需任何手动配置：混合架构 CPU（P+E 核）自动将推理线程绑定到性能核；认证期间自动解除 Windows 对后台进程的电源限流（EcoQoS），避免后台身份被压至低频；模型加载与相机图骨架在锁屏空闲期预载完成，触发后仅剩相机激活与推理。
