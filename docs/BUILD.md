...
.\build\tools\face_gain_tune\Release\FaceGainTuneTest.exe              # 暗处跨轮进度/重放、未稳定不保存、明暗转换
...
`FaceGainTuneTest` exercises the production exposure/gain controller with deterministic sensor measurements: a budget-limited dark round below the target band must persist its progress, replay must not reset it to the old value, following rounds must converge, and cancelled/lost-face/unsettled writes must not replace the checkpoint. It also checks measured gain response reaching an interior target within two writes, tolerance of next-round brightness variation, power-loss replay, extreme-dark exposure fallback and bright-room recovery. It also checks no-face white-out evidence (including bright-background rejection), bounded recovery, cancellation, unavailable controls, settle gating and face-tuning handoff without saving no-face settings. It does not replace real-camera timing verification.
...
The optional CSV replay uses the production settle gate and its sample cap. `unconfirmed` means the controller must stop adjusting without saving; it is not evidence of a dead actuator. The central ROI trace is a diagnostic proxy, not a substitute for face/PAD/identity and Session 0 validation. The probe is a development tool and is never copied into installer payloads.
